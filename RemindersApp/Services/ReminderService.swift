        ...
        switch statType {
            ...
            let today = Date()
                ...
                guard let tomorrow = tomorrow else {
                    fatalError("Error: Unable to get tomorrow's date")
                }
                let predicate = NSPredicate(format: "(reminderDate BETWEEN {%@, %@}) OR (reminderTime BETWEEN {%@, %@})", today as NSDate, tomorrow as NSDate, today as NSDate, tomorrow as NSDate)
